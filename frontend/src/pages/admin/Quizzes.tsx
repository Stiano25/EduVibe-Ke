...
import { ArrowLeft, Plus, Edit, Trash2, HelpCircle, Search } from 'lucide-react'
...
export const AdminQuizzes = () => {
  ...
  const [deleteModal, setDeleteModal] = useState<{ isOpen: boolean; quiz: Quiz | null }>({
    isOpen: false,
    quiz: null,
  })
  const [formModal, setFormModal] = useState<{ isOpen: boolean; quiz: Quiz | null }>({
    isOpen: false,
    quiz: null,
  })
  ...
  const filteredQuizzes = useMemo(() => {
    return quizzes.filter((quiz) => {
      ...
      if (filterGrade !== 'all' && quiz.grade !== filterGrade) return false
      if (filterDifficulty !== 'all' && quiz.difficulty !== filterDifficulty) return false
      return true
    })
  }, [quizzes, searchQuery, filterGrade, filterDifficulty])
  ...
  const handleAddClick = () => {
    setFormModal({ isOpen: true, quiz: null })
  }

  const handleEditClick = (quiz: Quiz) => {
    setFormModal({ isOpen: true, quiz })
  }
  ...
  return (
    <div className="min-h-screen premium-mesh" style={{ fontFamily: 'Manrope, sans-serif' }}>
                  ...
                  <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-text-secondary hidden sm:block" style={{ fontFamily: 'Manrope, sans-serif' }}>
                        Create, edit, and organize assessment quizzes
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {filteredQuizzes.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                  {filteredQuizzes.map((quiz) => (
                    <div
                      key={quiz.id}
                      className="bg-white/80 backdrop-blur-md rounded-[20px] border-2 border-slate-200 p-3 sm:p-4 hover:border-purple-300 transition-all"
                    >
      ...
      <DeleteModal
        ...
        title="Delete Quiz"
        description="This will permanently delete this quiz. This action cannot be undone."
      ...
      <QuizFormModal
        ...
        onClose={() => setFormModal({ isOpen: false, quiz: null })}
        onSave={handleSave}
        quiz={formModal.quiz}
        isLoading={isSaving}
      />
