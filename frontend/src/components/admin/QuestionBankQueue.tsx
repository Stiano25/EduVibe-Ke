...
import type { Grade, QuizQuestion, Strand, Subject, SubStrand } from '@/types'
...
export const QuestionBankQueue = () => {
  ...
  useEffect(() => {
    if (selectedSubject) fetchStrandsBySubject(selectedSubject.id)
  }, [selectedSubject, fetchStrandsBySubject])

  useEffect(() => {
    if (selectedStrand) fetchSubStrandsByStrand(selectedStrand.id)
  ...
  const loadEntries = async (subStrandId?: string | null) => {
    setLoading(true)
    setError(null)
    try {
  ...
  useEffect(() => {
    loadEntries(selectedSubStrand?.id)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [status, selectedSubStrand?.id])

  const startEdit = (entry: BankEntry) => {
    setEditingId(entry.id)
    setEditStem(entry.question?.question || '')
    setEditOptions([...(entry.question?.options || [])])
  ...
  const saveEdit = async (entry: BankEntry) => {
    ...
    try {
      ...
      setEditingId(null)
      await loadEntries(selectedSubStrand?.id)
    } catch (err) {
    ...
    <div className="rounded-[16px] border-2 border-indigo-100 bg-white/80 p-4 space-y-4">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600" style={{ fontFamily: 'Manrope, sans-serif' }}>
          AI-authored items for this sub-strand. Source PDFs above inform style only — never copy them.
          Approve here before lesson generation can pull them.
        </p>
      </div>
        ...
        {selectedGrade && (
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                const next = filteredSubjects.find((s) => s.id === e.target.value) || null
        ...
        {selectedSubject && (
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                const next = subjectStrands.find((s) => s.id === e.target.value) || null
                setSelectedStrand(next)
                setSelectedSubStrand(null)
              }}
              className="w-full px-3 py-2 rounded-[12px] border-2 border-slate-200 text-sm"
              style={{ fontFamily: 'Manrope, sans-serif' }}
        ...
        {selectedStrand && (
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                const next = strandSubStrands.find((s) => s.id === e.target.value) || null
                setSelectedSubStrand(next)
              }}
              className="w-full px-3 py-2 rounded-[12px] border-2 border-slate-200 text-sm"
              style={{ fontFamily: 'Manrope, sans-serif' }}
            >
