...
try {
  ...
  const shot = path.join(outDir, '04-after-topup-review.png');
  ...
  const snippets = bodyText
    ...
    .map((s) => s.trim())
    .filter((s) =>
      /flagged|coverage|remapped|QA|outcomes covered|Question bank|short —|Top up|QA flagged/i.test(s)
    )
    .slice(0, 40);
  console.log(JSON.stringify(snippets, null, 2));
} catch (e) {
  await page.screenshot({ path: path.join(outDir, '04-after-topup-error.png'), fullPage: true });
